...
function MeetingDetailClient() {
	...
	const { handleDeleteMeeting, DeleteConfirmDialog } = useDeleteMeetingDialog({
		onSuccess: () => router.push('/mypage/meetings/hosting'),
	});
	...
	// 신청 여부 직접 계산
	const isAlreadyAppliedByPending = pendingMeetings?.some(
		(meeting) => meeting.meetingId === meetingId,
	);

	// 수동 신청 상태 관리 (신청 직후 즉시 반영용)
	const [isPendingOverride, setIsPendingOverride] = useState(false);
	const isPending = isPendingOverride || isAlreadyAppliedByPending || false;
	...
	// 신청자 알림 토스트
	const { data: participations } = useParticipationsQuery(meetingId);
		...
		<div className="flex flex-col min-h-screen bg-background pt-12">
			<div className="flex-1 w-full max-w-5xl mx-auto pb-8 space-y-8 px-4 md:px-0">
				<div className="flex flex-col md:flex-row gap-8 md:gap-12">
					{/* 이미지 */}
					<div className="relative w-full md:w-1/2 aspect-square rounded-2xl overflow-hidden bg-muted shrink-0 shadow-sm border border-border/50">
						{isClosed && (
					...
					<div className="flex-1 flex flex-col h-full min-h-[500px] justify-between py-2">
								...
								{isHost && (
									<div className="ml-auto">
										<MeetingActionButtons
											...
											onDelete={() => handleDeleteMeeting(meetingId)}
										/>
									</div>
								)}
							</div>
						...
						<div className="mt-8 pt-6 border-t border-border/50">
							<MeetingActionButtons
								...
								location="detail-mid"
								isPending={isPending}
								isJoined={joinedMeetings?.some((m) => m.meetingId === meetingId)}
...
export default MeetingDetailClient;
