...
export const resetPasswordSchema = z
	.object({
		resetCode: z.string().min(1, '인증코드를 입력해주세요.'),
		newPassword: z.string().min(8, '비밀번호는 최소 8자 이상이어야 합니다.'),
		newPasswordConfirm: z.string().min(8, '비밀번호는 최소 8자 이상이어야 합니다.'),
...
const ResetPasswordClient = () => {
	...
	const handleVerify = async () => {
		...
		if (!resetCode) {
			setError('resetCode', { type: 'manual', message: '인증코드를 입력해주세요.' });
			return;
		}
	...
	const onSubmit = (data: ResetPasswordFormValues) => {
		...
		resetPassword(data, {
			...
			onError: (error: AxiosError) => {
				...
				setError('root', { type: 'manual', message: '비밀번호 변경에 실패했습니다.' });
			},
		});
	};
