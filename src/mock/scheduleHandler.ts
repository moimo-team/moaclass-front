...
import type { LessonSchedule, ScheduleParticipant } from '@/models/schedule.model';
...
export const scheduleHandlers = [
	http.get(`${httpUrl}/lessons/:lessonId/schedules`, ({ params }) => {
		...
		const filtered = mockSchedules.filter((s) => s.lessonId === Number(lessonId));
		return HttpResponse.json(filtered);
	}),

	http.post(`${httpUrl}/lessons/:lessonId/schedules`, async ({ request, params }) => {
		const { lessonId } = params;
	...
	http.get(`${httpUrl}/lessons/schedules/:scheduleId/participants`, ({ params }) => {
