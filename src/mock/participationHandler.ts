...
const getParticipationsManagement = http.get(
	`${httpUrl}/meetings/:meetingId/participations`,
	async ({ params }) => {
		await delay(1000);
...
const getParticipantsList = http.get(
	`${httpUrl}/meetings/:meetingId/participants`,
	async ({ params }) => {
		await delay(500);
...
// 개별 승인 핸들러
const approveParticipation = http.put(
	`${httpUrl}/meetings/:meetingId/participations/:participationId/approve`,
	async ({ params }) => {
		await delay(1000);
		...
		if (participant) {
			...
			// 인원수 업데이트
			meeting.currentParticipants = participants.filter(
				(p) => p.status === 'ACCEPTED',
			).length;
		}
...
// 개별 거절 핸들러
const rejectParticipation = http.put(
	`${httpUrl}/meetings/:meetingId/participations/:participationId/reject`,
	async ({ params }) => {
		await delay(1000);
		const { meetingId, participationId } = params;
		const mid = Number(meetingId);
...
// 전체 승인 핸들러
const approveAllParticipations = http.put(
	`${httpUrl}/meetings/:meetingId/participations/approve-all`,
	async ({ params }) => {
		await delay(1000);
		const { meetingId } = params;
		const mid = Number(meetingId);
...
// 승인 취소 핸들러
const cancelApprovalParticipation = http.put(
	`${httpUrl}/meetings/:meetingId/participations/:participationId/cancel-approval`,
	async ({ params }) => {
		await delay(1000);
		const { meetingId, participationId } = params;
		const mid = Number(meetingId);
		...
		if (participant) {
			...
			if (participant.status === 'ACCEPTED') {
				...
				return HttpResponse.json(
					{ message: '이미 PENDING 상태이거나 REJECTED인 경우 취소할 수 없습니다.' },
					{ status: 400 },
				);
			}
		}
...
// 거절 취소 핸들러
const cancelRejectParticipation = http.put(
	`${httpUrl}/meetings/:meetingId/participations/:participationId/cancel-rejection`,
	async ({ params }) => {
		await delay(1000);
		const { meetingId, participationId } = params;
		const mid = Number(meetingId);
		...
		if (participant) {
			...
			if (participant.status === 'REJECTED') {
				...
				return HttpResponse.json(
					{ message: '거절된 상태가 아니면 취소할 수 없습니다.' },
					{ status: 400 },
				);
			}
		}
