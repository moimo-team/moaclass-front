...
const checkEmail = http.post(`${httpUrl}/users/check-email`, async ({ request }) => {
	...
	if (email === 'taken@email.com') {
		return new HttpResponse(JSON.stringify({ message: '이미 사용 중인 이메일입니다.' }), {
			status: 409,
		});
	}
...
const findPassword = http.post(`${httpUrl}/users/password-reset/request`, async ({ request }) => {
	...
	return HttpResponse.json({ message: '이메일이 일치하지 않습니다.' }, { status: 404 });
});
...
const verifyResetCode = http.post(`${httpUrl}/users/password-reset/verify`, async ({ request }) => {
	...
	if (!storedData) {
		return new HttpResponse(JSON.stringify({ message: '유효하지 않은 인증코드입니다.' }), {
			status: 400,
		});
	}
	...
	if (storedData.email !== email) {
		return new HttpResponse(JSON.stringify({ message: '이메일이 일치하지 않습니다.' }), {
			status: 404,
		});
	}
	...
	if (storedData.expiresAt < new Date()) {
		...
		return new HttpResponse(JSON.stringify({ message: '인증코드가 만료되었습니다.' }), {
			status: 410,
		});
	}
...
const resetPassword = http.put(`${httpUrl}/users/password-reset/confirm`, async ({ request }) => {
	...
	if (!tokenData) {
		return new HttpResponse(JSON.stringify({ message: '유효하지 않은 토큰입니다.' }), {
			status: 400,
		});
	}
	...
	if (tokenData.expiresAt < new Date()) {
		...
		return new HttpResponse(JSON.stringify({ message: '토큰이 만료되었습니다.' }), {
			status: 410,
		});
	}
...
const refresh = http.post(`${httpUrl}/users/refresh`, async ({ request }) => {
	...
	return new HttpResponse(JSON.stringify({ message: '유효하지 않은 리프레시 토큰입니다.' }), {
		status: 401,
	});
});
	...
	return HttpResponse.json(
		{
			authenticated: true,
			isNewUser: false,
			...userStore.userInfo, // 모든 사용자 정보 포함
			accessToken: 'mock-jwt-token',
		},
		{
			status: 200,
...
const userUpdate = http.put(`${httpUrl}/users`, async ({ request }) => {
	try {
			...
			interests: interests.map((item: any, index: number) => {
				if (typeof item === 'number' || !isNaN(Number(item))) {
					...
					return {
						id: Number(item),
						name:
							LESSON_CATEGORIES.find((interest) => interest.id === Number(item))
								?.name || '',
					};
				}
		...
		if (file && file instanceof File) {
			updatedInfo.profileImage = URL.createObjectURL(file);
		}

		userStore.setUserInfo(updatedInfo);

		await delay(1000);
		return HttpResponse.json(userStore.userInfo);
	} catch (error) {
