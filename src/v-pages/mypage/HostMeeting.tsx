...
const MeetingCardWithBadge = ({
	...
	if (isLoading) {
		return (
			<div className="w-full h-[108px] bg-gray-50 animate-pulse rounded-xl border border-gray-100" />
		);
	}

	return (
		<SmallMeetingCard meeting={meeting} hasPendingApplicants={hasPendingApplicants}>
...
const HostMeeting = () => {
	...
	const {
		...
		totalPages,
		isLoading,
	} = useMeQuery('hosted', 'all', page, 5);
	const { handleDeleteMeeting, DeleteConfirmDialog } = useDeleteMeetingDialog();

	return (
		<div className="w-full h-full py-6 lg:py-10 bg-white overflow-y-auto">
			<h1 className="text-2xl font-bold text-gray-900 mb-6 lg:mb-8">내 모임</h1>
