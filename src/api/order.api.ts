...
// 수강취소 내역 조회
export const getCancelClass = async (id: number): Promise<CancelClassResponse> => {
	try {
		const response = await apiClient.get<CancelClassResponse>(`/enrollments/${id}/cancel`);
		return response.data;
	} catch (error) {
...
// 수강취소
export const cancelClass = async (id: number, data: CancelClassRequest): Promise<void> => {
	try {
