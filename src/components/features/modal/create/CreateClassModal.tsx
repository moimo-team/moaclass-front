...
function CreateClassModal({ open, onOpenChange, classId }: CreateClassModalProps) {
  ...
  useEffect(() => {
    if (open) {
      if (classId && existingLesson) {
        ...
        reset({
          title: "테스트 클래스",
          description: "이것은 테스트용 클래스 설명입니다. 실제 서비스에서는 이 내용을 수정해주세요.",
          curriculum: "1. 첫 번째 커리큘럼 내용입니다.\n2. 두 번째 커리큘럼 내용입니다.\n3. 세 번째 커리큘럼 내용입니다.",
  ...
  useEffect(() => {
    if (selectedCategoryId && prevCategoryIdRef.current !== null && prevCategoryIdRef.current !== selectedCategoryId) {
  ...
  const onSubmit = async (data: ClassFormValues) => {
    try {
      ...
      formData.append("discountedPrice", Math.round(data.price * (1 - data.discountRate / 100)).toString());
  ...
  return (
      ...
      >
            ...
            <FormField label="소분류 카테고리" description="클래스의 소분류 카테고리를 선택해주세요 (복수 선택 가능)" required>
              <div className="flex flex-wrap gap-2">
                {subCategories.map((subCategory) => {
                  ...
                  console.log(`  - ${subCategory.name} (ID: ${subCategory.id}): ${isSelected ? '✅ 선택됨' : '❌ 선택안됨'}`);
                  return (
                    <SelectableBadge
                      key={subCategory.id}
        ...
        {/* 소요 시간 */}
        <FormField label="소요 시간" description={`${duration}분 (${Math.floor(duration / 60)}시간 ${duration % 60}분)`} required>
          <Controller
            name="duration"
            control={control}
            render={({ field }) => (
