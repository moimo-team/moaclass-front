...
export const ClassManageCard = ({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "ACTIVE":
        return <Badge variant="active" className="px-2.5 py-1 text-xs font-bold rounded-lg shadow-sm">운영중</Badge>;
      case "INACTIVE":
        return <Badge variant="inactive" className="px-2.5 py-1 text-xs font-bold rounded-lg shadow-sm bg-gray-400 text-white border-none">휴면</Badge>;
      case "DRAFT":
        return <Badge variant="secondary" className="px-2.5 py-1 text-xs font-bold rounded-lg shadow-sm">임시저장</Badge>;
      default:
        return <Badge variant="outline" className="px-2.5 py-1 text-xs font-bold rounded-lg">{status}</Badge>;
    }
  };
        ...
        <div className="absolute top-3 right-3 z-10 ">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-9 w-9 bg-white/90 hover:bg-white  shadow-md backdrop-blur-sm transition-all active:scale-90">
                <MoreVertical className="h-4.5 w-4.5 text-gray-700" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="border-gray-100 p-1.5 shadow-xl ">
              <DropdownMenuItem onClick={onDuplicate} className="cursor-pointer  font-medium">클래스 복제</DropdownMenuItem>
              <DropdownMenuItem onClick={onToggleStatus} className="cursor-pointer font-medium">
                {lesson.status === "ACTIVE" ? "휴면" : "휴면 해제"}
              </DropdownMenuItem>
              <div className="h-px bg-gray-100 my-1" />
              <DropdownMenuItem onClick={onDelete} className="text-destructive cursor-pointer font-bold">
                삭제
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
