...
const ScheduleCard = ({
	...
	return (
		<div
			...
			className={`relative p-4 border rounded-xl transition-all duration-300 group ${
				isWithParticipants
		...
		>
			<div className="flex flex-col gap-2.5">
				<div className="flex items-center justify-between">
					<div
						className={`flex items-center gap-1.5 text-[14px] font-black tracking-tight ${isSelected ? 'text-primary' : 'text-gray-800'}`}
					>
				...
				<div className="flex items-center justify-between bg-gray-50/50 p-2 rounded-lg">
					<div
						className={`flex items-center gap-2 text-[13px] font-bold ${isSelected ? 'text-gray-800' : 'text-gray-500'}`}
					>
						<Clock
							className={`w-3.5 h-3.5 ${isWithParticipants ? 'text-carrot/80' : isSelected ? 'text-primary' : 'text-primary/30'}`}
						/>
						{extractTimeFromISO(schedule.startAt)} -{' '}
						{extractTimeFromISO(schedule.endAt)}
					</div>

					<div
						className={`text-[12px] font-black ${isWithParticipants ? 'text-carrot' : isSelected ? 'text-primary/80' : 'text-primary/40'}`}
					>
