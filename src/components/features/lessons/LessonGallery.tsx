...
export const LessonGallery = ({ title, lessonImages }: LessonGalleryProps) => {
	...
	const goToPrevImage = () => {
		...
		setActiveIndex((prevIndex) => (prevIndex === 0 ? lessonImages.length - 1 : prevIndex - 1));
	};

	const goToNextImage = () => {
		if (!lessonImages || lessonImages.length <= 1) return;

		setActiveIndex((prevIndex) => (prevIndex === lessonImages.length - 1 ? 0 : prevIndex + 1));
	};
